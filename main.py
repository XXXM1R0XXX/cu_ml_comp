...
def _(Pool, test, train_df, val_df):
    train_pool = Pool(data=train_df.drop(['a6_flg'], axis=1), label=train_df['a6_flg'], cat_features=['product'], timestamp=train_df['month_dt'])
    val_pool = Pool(data=val_df.drop(['a6_flg'], axis=1), label=val_df['a6_flg'], cat_features=['product'], timestamp=val_df['month_dt'] )
